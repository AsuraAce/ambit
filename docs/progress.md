...
- ComfyUI metadata milestones 22 through 53 are complete on the unmerged integration stack. The pinned v0.11.18 target assesses all 93 workflows: 80 golden, 5 pattern-covered, 8 partial, and 0 unassessed. Milestone 53 refreshes every changed targeted workflow, adds direct evidence for the bypassed `gsl_starter_1_3` pattern, and resolves modern subgraph instance widgets that omit `proxyWidgets`. The path-parity gate now covers all 90 official catalog fixtures and 21 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version is 42; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
