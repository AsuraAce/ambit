...
- ComfyUI metadata milestones 22 through 46 are complete on the unmerged integration stack. The pinned v0.11.15 target now assesses all 93 workflows: 80 golden, 5 pattern-covered, 8 partial, and 0 unassessed. Milestone 46 closes the official image use cases while preserving the eight-output camera-angle workflow as an honest partial. Parser version remains 37 because milestones 45 and 46 add coverage without changing extraction behavior. Their files under `docs/plans/` are historical verification records, not active work.
