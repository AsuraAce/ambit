...
- ComfyUI metadata milestones 22 through 56 are complete on the unmerged integration stack. The pinned v0.11.18 target assesses all 93 workflows: 83 golden, 5 pattern-covered, 5 partial, and 0 unassessed. Milestone 53 refreshes the catalog and resolves modern subgraph instance widgets that omit `proxyWidgets`; Milestone 54 adds an offline read-only audit for catalog IDs, upstream blob identities, candidate drift, and all 90 dedicated fixtures; Milestone 55 adds privacy-safe Developer Mode output-selection and traversal-blocker diagnostics; Milestone 56 locks exact diagnostic contracts for the remaining partials and corrects three stale classifications from selected-path evidence. GSC uses its literal sampler prompt, while both HiDream O1 workflows select literal prompt branches through conditioning switches. The path-parity gate covers all 90 official catalog fixtures and 21 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version is 43; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
