...
- ComfyUI metadata milestones 22 through 55 are complete on the unmerged integration stack. The pinned v0.11.18 target assesses all 93 workflows: 80 golden, 5 pattern-covered, 8 partial, and 0 unassessed. Milestone 53 refreshes the catalog and resolves modern subgraph instance widgets that omit `proxyWidgets`; Milestone 54 adds an offline read-only audit for catalog IDs, upstream blob identities, candidate drift, and all 90 dedicated fixtures; Milestone 55 adds privacy-safe Developer Mode output-selection and traversal-blocker diagnostics without changing extraction behavior. The audit also corrected one stale `templates-image_to_real` fixture while preserving its golden parser output. The path-parity gate covers all 90 official catalog fixtures and 21 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version is 42; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
