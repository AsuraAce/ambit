...
- ComfyUI metadata milestones 22 through 52 are complete on the unmerged integration stack. The pinned v0.11.15 target assesses all 93 workflows: 80 golden, 5 pattern-covered, 8 partial, and 0 unassessed. Milestone 52 consumes persisted cg-use-everywhere resolved links without simulating the plugin's matching engine, preserving real-edge authority and conservative failure behavior. The path-parity gate now covers all 89 official catalog fixtures and 21 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version is 41; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
