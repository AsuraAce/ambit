...
- ComfyUI metadata milestones 22 through 57 are complete on the unmerged integration stack. The pinned v0.11.18 target assesses all 93 workflows: 86 golden, 2 pattern-covered, 5 partial, and 0 unassessed. Milestone 54 adds an offline read-only audit for catalog IDs, upstream blob identities, candidate drift, and dedicated fixtures; Milestone 55 adds privacy-safe Developer Mode output-selection and traversal-blocker diagnostics; Milestone 56 locks exact diagnostic contracts for the remaining partials; and Milestone 57 gives every active target direct pinned fixture evidence. The two remaining pattern-covered entries are intentionally disconnected or bypassed starter workflows. The path-parity gate covers all 93 official catalog fixtures and 21 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version is 43; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
