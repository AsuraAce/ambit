...
- ComfyUI metadata milestones 22 through 59 are complete on the unmerged integration stack. The pinned v0.11.18 core target remains all 93 workflows: 86 golden, 2 pattern-covered, 5 partial, and 0 unassessed. Milestones 58 and 59 add a separate eight-workflow extended image scope for official utility and custom-node workflows; all eight are golden, bringing the whole manifest to 94 golden, 2 pattern-covered, 5 partial, and 477 excluded. The two remaining pattern-covered entries are intentionally disconnected or bypassed starter workflows. The path-parity gate covers 101 official catalog fixtures and 21 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version is 44; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
