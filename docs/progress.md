...
- ComfyUI metadata milestones 22 through 49 are complete on the unmerged integration stack. The pinned v0.11.15 target assesses all 93 workflows: 80 golden, 5 pattern-covered, 8 partial, and 0 unassessed. Milestone 49 preserves duplicate workflow input slots and applies role-aware positive and negative routing for legacy `Prompts Everywhere` nodes. The path-parity gate now covers all 89 official catalog fixtures and 15 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version is 39 for the wireless-prompt correction; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
