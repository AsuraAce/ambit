...
- ComfyUI metadata milestones 22 through 45 are complete on the unmerged integration stack. The pinned v0.11.15 core target remains fully assessed, and milestone 45 expands the active catalog target to 93 workflows: 75 golden, 5 pattern-covered, 7 partial, and 6 unassessed official image use cases. Parser version remains 37 because milestone 45 adds coverage without changing extraction behavior. Their files under `docs/plans/` are historical verification records, not active work.
