...
- InvokeAI owner-scope startup remediation is implementation-complete and release-verified as a separate four-package workstream. Work Package 1 adds a truthful blocking preparation gate, real reconciliation progress, an unchanged-scope fast path, and authoritative library counts. Work Package 2 adds in-context owner selection, fail-closed trusted offline recovery, persistent warnings, actionable errors, and automatic catch-up after recovery. Work Package 3 prevents library-derived work before owner admission, makes reconciliation scale with keyset paging and cancellable batches, exposes its expensive phases, and presents database and InvokeAI preparation as one accessible startup journey. Work Package 4 closes the startup presentation handoff: brief database, owner, and privacy-protection checks stay behind the branded splash; sustained work replaces the splash after a grace period; and each revealed preparation stage observes its own minimum display time. Selection, errors, and post-startup owner changes remain immediate. The real-data desktop restart is owner-accepted; multi-owner selection and temporary-offline retry remain recovery acceptance for Work Packages 2 and 3. See `docs/plans/invokeai-owner-scope-startup-remediation.md`.
...
## Next Work
- Complete the remaining owner-acceptance recovery journey for InvokeAI owner-scope startup remediation Work Packages 2 and 3: multi-owner selection, temporary offline startup, retry, and scope change without missing or deleted Ambit records. The Work Package 4 normal-restart presentation is accepted.
