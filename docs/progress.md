...
- ComfyUI metadata milestones 22 through 51 are complete on the unmerged integration stack. The pinned v0.11.15 target assesses all 93 workflows: 80 golden, 5 pattern-covered, 8 partial, and 0 unassessed. Milestone 51 normalizes GGUF model names and recognizes case-insensitive GGUF UNET loaders across connected traversal and conservative wireless fallback. The path-parity gate now covers all 89 official catalog fixtures and 20 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version is 40; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
