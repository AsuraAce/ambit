...
- ComfyUI metadata milestones 22 through 48 are complete on the unmerged integration stack. The pinned v0.11.15 target assesses all 93 workflows: 80 golden, 5 pattern-covered, 8 partial, and 0 unassessed. Milestone 48 adds four minimized real-world custom-node fixtures plus exact `KSampler //Inspire` workflow-widget support. The path-parity gate now covers all 89 official catalog fixtures and 14 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version is 38 for the Inspire extraction correction; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
