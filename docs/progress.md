...
- ComfyUI metadata milestones 22 through 50 are complete on the unmerged integration stack. The pinned v0.11.15 target assesses all 93 workflows: 80 golden, 5 pattern-covered, 8 partial, and 0 unassessed. Milestone 50 moves the remaining placeholder-prompt and loader-only legacy repro shapes into exact fixture and provenance coverage. The path-parity gate now covers all 89 official catalog fixtures and 19 real-world fixtures across direct extraction, scanner-style merging, reparse, and developer diagnostics. Parser version remains 39; the integration stack remains intentionally unmerged so users receive one later metadata refresh rather than repeated refreshes.
