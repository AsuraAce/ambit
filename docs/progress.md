...
- ComfyUI metadata milestones 22 through 47 are complete on the unmerged integration stack. The pinned v0.11.15 target assesses all 93 workflows: 80 golden, 5 pattern-covered, 8 partial, and 0 unassessed. Milestone 47 adds a path-parity gate over all 89 official catalog fixtures and 10 real-world fixtures so direct extraction, scanner-style merging, reparse, and developer diagnostics cannot drift silently. Parser version remains 37 because milestones 45 through 47 add coverage without changing extraction behavior. Their files under `docs/plans/` are historical verification records, not active work.
