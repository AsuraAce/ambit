...
*   **TAURI_SIGNING_PRIVATE_KEY (Required for updater-enabled releases):**
    *   **What it is:** The full contents of the private updater signing key.
    *   **Why it's important:** Tauri's updater verifies release bundles against the public key embedded in [`src-tauri/tauri.conf.json`](/C:/Users/Artemis/.codex/worktrees/6742/project-ambit-alpha/src-tauri/tauri.conf.json). The release workflow now fails if this secret is missing.
    *   **Current local key path:** `C:\Users\Artemis\.codex\worktrees\6742\project-ambit-alpha\.tauri\ambit-updater.key`
    *   **How to add it:** Copy the entire file contents into the `TAURI_SIGNING_PRIVATE_KEY` repository secret.
*   **TAURI_SIGNING_PRIVATE_KEY_PASSWORD (Optional):**
    ...
    *   **Current state:** The locally generated Ambit updater key has no password, so this secret can remain empty unless the key is rotated to a protected one later.
...
*   A local keypair was generated with `pnpm tauri signer generate -- --ci -w .tauri/ambit-updater.key`.
*   The public key is embedded in `src-tauri/tauri.conf.json`.
*   The private key is intentionally ignored by Git and should also be backed up somewhere durable outside GitHub secrets.
...
*   **Updater signing:** `TAURI_SIGNING_PRIVATE_KEY` must be present before any release tag is built.
