...
Smart Thumbnail reconciliation distinguishes three outcomes. A readable source can be optimized or reused; an individually absent source is marked `is_missing` without deleting its cached thumbnail or database row; and an unavailable InvokeAI `outputs/images` root is treated as temporarily offline and skipped. Native bulk probes report present, missing, and inspection-error outcomes separately, so zero-byte or unreadable files are never inferred missing from a size sentinel. Invoke offline-root handling uses the row's registered source identity and configured images root rather than path-shape heuristics. Missing rows leave the optimizer candidate set and can be restored by a later successful InvokeAI sync. Retry diagnostics are operational metadata and do not invalidate owner-scope caches; bulk missing reconciliation invalidates each affected scope once per committed batch, and the optimizer's v2 queue index covers its equality predicates and keyset order. The supervising hook owns cancellation and Activity Dock cleanup.
