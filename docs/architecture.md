...
Smart Thumbnail reconciliation distinguishes three outcomes. A readable source can be optimized or reused; an individually absent source is marked `is_missing` without deleting its cached thumbnail or database row; and a temporarily unavailable managed source root is skipped. The optimizer receives active monitored roots from the frontend, derives InvokeAI `outputs/images` roots from registered source identity, and caches unavailable roots before inspecting child files. Root probes are time-bounded, cancellation-aware, and process-wide single-flight so an OS-stalled probe cannot accumulate workers across retries; a chunk containing absent children rechecks each affected root once before persistence so a drive disconnected during the run is deferred instead of marked missing. Native bulk probes report present, missing, and inspection-error outcomes separately, so zero-byte or unreadable files are never inferred missing from a size sentinel; their IPC boundary also rejects paths outside the registered filesystem scope. Missing rows leave the optimizer candidate set and can be restored by a later successful InvokeAI sync. Retry diagnostics are operational metadata and do not invalidate owner-scope caches; bulk missing reconciliation invalidates each affected scope once per committed batch, and the optimizer's v2 queue index covers its equality predicates and keyset order. The supervising hook owns cancellation and Activity Dock cleanup.
