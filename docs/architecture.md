...
Smart Thumbnail reconciliation distinguishes three outcomes. A readable source can be optimized or reused; an individually absent source is marked `is_missing` without deleting its cached thumbnail or database row; and an unavailable InvokeAI `outputs/images` root is treated as temporarily offline and skipped. Missing rows leave the optimizer candidate set and can be restored by a later successful InvokeAI sync. Retry diagnostics are operational metadata and do not invalidate owner-scope caches; bulk missing reconciliation invalidates each affected scope once per committed batch. The supervising hook owns cancellation and Activity Dock cleanup.
