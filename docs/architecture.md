...
Smart and Maintenance thumbnail repair share one native engine for source-root validation, generation, outcome persistence, cancellation, and diagnostics. Canonical required and upgradeable views use disjoint partial indexes and keyset ordering. Smart discovers those views incrementally without a startup count, respects failure backoff, and changes between normal and throttled orchestration around foreground activity. Maintenance keeps its exact scoped count and keyset pages, submits explicit image IDs at Fast priority, and bypasses failure backoff; Regenerate Selected additionally forces a real overwrite rather than accepting a cached file. One native job lease prevents Smart and Maintenance from processing concurrently.
