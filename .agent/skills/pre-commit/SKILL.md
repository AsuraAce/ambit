...
```bash
# 1. Type Check TypeScript (Catch compilation errors before they are committed)
npx tsc --noEmit

# 2. Frontend Unit Tests (Use 'run' to prevent watch mode from hanging the commit)
npx vitest run
...
```
It user says yes, write the rule to the appropriate file and update the entry status to `graduated`.
