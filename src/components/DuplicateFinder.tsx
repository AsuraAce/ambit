...
import { AlertTriangle, Check, Layers, Copy, ArrowRight, Maximize2 } from 'lucide-react';
...
export const DuplicateFinder: React.FC<DuplicateFinderProps> = ({ images, onResolve, onStack }) => {
                        ...
                        {group.images.map((img, idx) => (
                            <div key={img.id} className="group relative flex flex-col min-w-[160px] w-[calc(50%-0.5rem)] flex-shrink-0">
                                ...
                                <div className="relative aspect-[2/3] bg-gray-100 dark:bg-slate-950 rounded-lg overflow-hidden border border-gray-200 dark:border-white/10 group-hover:border-sage-500/50 transition-colors">
                                    ...
                                    {/* Resolution Badge */}
                                    <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-md text-[10px] font-mono text-white px-2 py-0.5 rounded border border-white/10 shadow-sm">
