    ...
    <div className="w-[420px] flex flex-col h-full bg-white dark:bg-zinc-900/95 backdrop-blur-xl border-l border-gray-200 dark:border-white/10 shadow-2xl">
        ...
        <div className="flex border-b border-gray-200 dark:border-white/5 shrink-0 bg-white dark:bg-zinc-900 p-2 gap-2">
            {(['info', 'edit', 'workflow'] as const).map(tab => (
                (tab !== 'workflow' || image.metadata.workflowJson) && (
                    <button 
                        key={tab}
                            ...
                            {isGenDataOpen && (
                                <div className="p-4 space-y-3 animate-in slide-in-from-top-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        <ParamItem label="Sampler" value={image.metadata.sampler} isModified={isModified('sampler')} />
                                        <ParamItem label="Steps" value={image.metadata.steps.toString()} isModified={isModified('steps')} />
                                        <ParamItem label="CFG Scale" value={image.metadata.cfg.toString()} isModified={isModified('cfg')} />
                                        <ParamItem label="Seed" value={image.metadata.seed.toString()} fullWidth isModified={isModified('seed')} />
                                    </div>
                                </div>
                            )}
                        </div>
