...
pub fn generate_thumbnail(path: &str, thumbnail_dir: &str) -> Result<ThumbnailResult, String> {
    generate_thumbnail_with_force(path, thumbnail_dir, false)
}

pub fn generate_thumbnail_with_force(
    path: &str,
    thumbnail_dir: &str,
    force: bool,
    ...
    let generated_thumbnail_path = if thumb_path.exists() && !force {
...
mod tests {
    ...
    #[test]
    fn force_regeneration_overwrites_an_existing_thumbnail() {
        ...
        let regenerated = generate_thumbnail_with_force(
            source_path.to_str().expect("source path"),
            thumbnail_dir.to_str().expect("thumbnail path"),
            true,
        )
