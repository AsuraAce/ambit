...
fn manifest_counts_match_the_declared_catalog_scope() {
    ...
    assert_eq!(manifest.counts.extended_target_entries, 12);
    assert_eq!(manifest.counts.excluded_entries, 473);
...
fn every_measured_target_has_dedicated_fixture_evidence() {
    ...
    assert_eq!(extended_targeted.len(), 12);
