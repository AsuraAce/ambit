...
impl DbPool {
    ...
    pub(crate) async fn select_traced(
        &self,
        query_text: String,
        parameters: Vec<JsonValue>,
        trace: &mut crate::startup_trace::Span,
    ) -> Result<Vec<IndexMap<String, JsonValue>>, crate::Error> {
        use crate::startup_trace::Stage;
        // Only SQLite is split into stages. Other enabled drivers retain upstream behavior.
        #[cfg(feature = "sqlite")]
        #[allow(irrefutable_let_patterns)]
        if let DbPool::Sqlite(pool) = self {
            trace.stage(Stage::Bind);
            ...
            trace.stage(Stage::Acquire);
            let mut connection = pool.acquire().await?;
            trace.stage(Stage::Fetch);
            let rows = (&mut *connection).fetch_all(query).await?;
            // Match Pool::fetch_all's return of its checked-out connection before decoding.
            drop(connection);
            trace.stage(Stage::Decode);
        ...
        self.select(query_text, parameters).await
    }
